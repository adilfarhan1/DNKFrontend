...
export const HeaderProject = () => {
  ...
  const [logoSwitch, setLogoSwitch] = useState(dnkLogo);
  ...
  useEffect(() => {
    if (projectData?.developerlogo) {
      setLogoSwitch(`${URL}${projectData.developerlogo}`);
    } else {
      setLogoSwitch(dnkLogo);
    }
  }, [projectData]);
  ...
  const fetchProject = async () => {
    try {
      ...
      if (response.success) {
        ...
        if (!projectData) setError("No project found with the provided ID.");
      } else {
