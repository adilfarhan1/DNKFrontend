...
export const ADContactForm = ({ onFormSubmit }) => {
  ...
  const addLeadtobitrix = async (name, email, phone, city) => {
    ...
    const leadData = {
      fields: {
        TITLE: "Website Lead Emaar Grand Polo Club & Resort",
        NAME: name,
        PHONE: [
          {
            VALUE: phone,
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div className="flex flex-col justify-between mx-[0px] md:mx-[10px] mt-3 md:mt-0">
        ...
        <div>
          <button
            className="bg-[#000000] hover:bg-[#CFA028] text-[#ffffff] hover:text-[#ffffff] w-full p-[10px] mt-[25px] rounded duration-100 flex justify-center"
            disabled={loading}
          >
            {loading ? (
