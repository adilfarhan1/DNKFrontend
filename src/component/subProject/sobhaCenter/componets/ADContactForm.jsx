...
export const ADContactForm = ({ onFormSubmit }) => {
  ...
  const addLeadtobitrix = async (name, email, phone, city) => {
    ...
    const leadData = {
      fields: {
        TITLE: "Sobha Center Website Lead",
        NAME: name,
        PHONE: [
          {
            VALUE: phone,
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div className="flex flex-col justify-between mx-[0px] md:mx-[10px] mt-3 md:mt-0">
        ...
        <div>
          <button
            className="bg-[#000000] hover:bg-[#0D84C8] text-[#ffffff] hover:text-[#ffffff] w-full p-[10px] mt-[25px] rounded duration-100 flex justify-center"
            disabled={loading}
          >
            {loading ? (
          ...
          <div className="flex items-center justify-center mt-4">
            <p className="mb-0 text-[#000000]">Or contact us right now via</p>
            <a
            ...
            >
              <FaWhatsapp className="text-[#0072B2] text-[1.3rem] ml-2 group-hover:text-[#6B9B2D]" />
              <p className="mb-0 !text-[#0072B2] group-hover:!text-[#6B9B2D]">
                WhatsApp
              </p>
            </a>
          </div>
